...
struct RecipeDetailScreen: View {
    ...
    var body: some View {
        if let recipe = state.recipe(recipeId) {
            content(for: recipe)
                ...
                // GeekBreadApp's `.onContinueUserActivity` listens for,
                ...
                .alert("Delete \(recipe.title)?", isPresented: $showDeleteConfirm) {
                    Button("Delete", role: .destructive) {
                        // AppState.deleteRecipe routes back to the library
                        // when the open recipe is the one being deleted,
                        // so the user lands somewhere coherent after the
                        // confirmation dismisses.
                        state.deleteRecipe(id: recipeId)
                    }
                    Button("Cancel", role: .cancel) {}
                } message: {
                    Text("This can't be undone, and it'll also remove the recipe from your other devices.")
                }
        } else {
    ...
    private func detailHeader(recipe: Recipe) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            .padding(.top, 8)
            if case .linked(_, let name, _) = recipe.source {
                Text("Recipe by \(name). GeekBread extracted the formula for scheduling; full instructions live at the source.")
                    .font(Typography.ui(13))
                    .foregroundStyle(Theme.slate600)
                    .padding(.top, 8)
            }
