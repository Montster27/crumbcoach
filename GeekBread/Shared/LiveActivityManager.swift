...
//
// Mac Catalyst doesn't ship ActivityKit, so the entire implementation is
// gated. AppState still calls `start` / `update` / `end` — on Catalyst they
// become no-ops so callers don't need #if guards everywhere.
