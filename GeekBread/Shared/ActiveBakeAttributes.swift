...
//
// ActivityKit isn't available on Mac Catalyst, so the `ActivityAttributes`
// conformance is gated. The plain data shape still compiles everywhere so
// `LiveActivityManager`'s public surface keeps working as a no-op.

struct ActiveBakeAttributes {
...
#if !targetEnvironment(macCatalyst)
extension ActiveBakeAttributes: ActivityAttributes {}
#endif
