...
// disk in `<App Support>/GeekBread/telemetry/` so the user can share them
...
final class TelemetryManager: NSObject {
    ...
    private let log = Logger(subsystem: "com.geekbread.app", category: "telemetry")
    ...
    private override init() {
        ...
        let dir = base
            .appendingPathComponent("GeekBread", isDirectory: true)
    ...
    func diagnosticReportText() -> String? {
        ...
        let header = "GeekBread diagnostic report\nGenerated \(ISO8601DateFormatter().string(from: Date()))\nPayloads: \(files.count)\n\n"
