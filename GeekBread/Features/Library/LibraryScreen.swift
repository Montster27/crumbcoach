...
struct LibraryScreen: View {
    ...
    /// `geekbread://import?url=…` deep link from the share extension,
    ...
    /// The recipe the user has selected for deletion from a long-press
    /// context menu, or `nil` when no confirmation alert is up.
    @State private var pendingDelete: Recipe? = nil
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            HStack(spacing: 24) {
                ...
                statCount(sourceTotals.original, "GeekBread originals")
        ...
        .alert("Delete \(pendingDelete?.title ?? "this recipe")?",
