...
//
// We export FORMULA + STAGES, not narrative prose. CrumbCoach is the user's
...
enum RecipeExporter {
    ...
    static func markdown(for recipe: Recipe, units: Units = .grams) -> String {
        ...
        lines.append("---")
        lines.append("")
        lines.append("Open in CrumbCoach: \(deepLink(for: recipe).absoluteString)")
    ...
    /// recipe inside CrumbCoach. Format: `crumbcoach://recipe/<id>`.
    static func deepLink(for recipe: Recipe) -> URL {
        URL(string: "crumbcoach://recipe/\(recipe.id)")
            ?? URL(string: "crumbcoach://recipe")!
    }

    /// Deep link used by the share extension to hand a Safari page URL back
    /// to the main app's recipe importer. Format:
    /// `crumbcoach://import?url=<percent-encoded>`.
